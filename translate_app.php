...
$targets = ["hi" => "hi", "te" => "te"]; // Truncated list for demonstration

function translate_text($text, $target) {
    // Note: Python's deep_translator uses web scraping, PHP requires API key via Guzzle/CURL.
...
foreach ($targets as $code => $gcode) {
    ...
    if (file_exists($out_file) && filesize($out_file) > 100) {
        ...
        if ($xml !== false) {
            foreach ($xml->string as $str) {
                if (isset($str['name'])) {
                    $existing[(string)$str['name']] = (string)$str;
                }
            }
        }
    }
    ...
    foreach ($tree->string as $strNode) {
        ...
        if (isset($existing[$name]) && $existing[$name] !== $text) {
            $strNode[0] = $existing[$name];
            continue;
        }
